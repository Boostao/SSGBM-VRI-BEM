...
add_std_crown_fields <- function(vri) {
  ...
  vri[ , pct_in_species_1 := (SPECIES_CD_1 %in% b_species) * as.numeric(SPECIES_PCT_1)]
  vri[ , pct_in_species_2 := (SPECIES_CD_2 %in% b_species) * as.numeric(SPECIES_PCT_2)]
  vri[ , pct_in_species_3 := (SPECIES_CD_3 %in% b_species) * as.numeric(SPECIES_PCT_3)]
  vri[ , pct_in_species_4 := (SPECIES_CD_4 %in% b_species) * as.numeric(SPECIES_PCT_4)]
  vri[ , pct_in_species_5 := (SPECIES_CD_5 %in% b_species) * as.numeric(SPECIES_PCT_5)]
  vri[ , pct_in_species_6 := (SPECIES_CD_6 %in% b_species) * as.numeric(SPECIES_PCT_6)]
