vribem_corrections_view <- function(conn, beu_bec, clear_site_ma = TRUE, use_ifelse = TRUE){
  ...
  add_col_to_tbl(conn, tbl_name = "VRIBEM_CORRECTIONS", col = "lbl_edit", type = "VARCHAR DEFAULT ''")
  add_col_to_tbl(conn, tbl_name = "VRIBEM_CORRECTIONS", col = "DEC_Total", type = "INTEGER DEFAULT 0")
  add_col_to_tbl(conn, tbl_name = "VRIBEM_CORRECTIONS", col = "SMPL_TYPE", type = "VARCHAR DEFAULT NULL")
  add_col_to_tbl(conn, tbl_name = "VRIBEM_CORRECTIONS", col = "row_updated", type = "BOOLEAN DEFAULT FALSE")
  add_col_to_tbl(conn, tbl_name = "VRIBEM_CORRECTIONS", col = "blank_eco_variables", type = "BOOLEAN DEFAULT FALSE")
  ...
  combine_duplicated_BEUMC_view(conn, view_name = "VRIBEM_CORRECTIONS")
  ...
  remove_inadequate_wetlands_view(conn, view_name = "VRIBEM_CORRECTIONS")
  ...
  duckdb::dbSendQuery(conn, 
    sprintf("UPDATE %s 
      SET SDEC_1 = %s,
       ...
       blank_eco_variables = TRUE
      WHERE %s;", 
    view_name, sdec_1, beumc_s1, lbl_edit, lbl_edit, cond)
  )
}

remove_inadequate_wetlands_view <- function(conn, view_name = "VRIBEM_CORRECTIONS"){
