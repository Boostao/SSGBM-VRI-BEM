...
init_db <- function(dbdir = defdb(),
  ...
  if (!is.null(bem_dsn) || duckdb::dbExistsTable(conn, "BEM")) {
    ...
    logger::log_info("See `?init_bem` to initialize BEM data.")
  }
  init_wetlands(conn, ask = ask)
  init_rivers(conn, ask = ask)
  init_lakes(conn, ask = ask)
  init_glaciers(conn, ask = ask)
...
#'   connection. When dsn is left empty/NULL the polygons will be fetched from
#'   the BC Data Catalog.
#' @param layer layer name (varies by driver, may be a file name without
#'   extension); in case layer is missing, st_read will read the first layer of
#'   dsn, give a warning and (unless quiet = TRUE) print a message when there
#'   are multiple layers, or give an error if there are no layers in dsn. If dsn
#'   is a database connection, then layer can be a table name or a database
#'   identifier (see Id). It is also possible to omit layer and rather use the
#'   query argument.
...
init_bem <- function(conn = init_conn(),
  ...
  if (!file.exists(dsn)) {
    logger::log_error(sprintf("Provided value for dsn does not exist [%s].", dsn))
  }
...
init_generic <- function(conn = init_conn(),
  ...
  if (duckdb::dbExistsTable(conn, tablename)) {
    ...
    if (interactive() && ask) {
      ...
      logger::log_info(
        "Non-interactive session detected. Skipping %s initialization." |>
          sprintf(tablename)
      )
      return(invisible())
    }
  ...
  if (is.null(dsn)) {
    select1 <- if (length(.include)) {
      \(x) {
        bcdata::select(x, .include = .include)
      }
    } else {
      bcdata::select
    }
    dsn <- bcdata::bcdc_query_geodata(record = recordid) |>
      select1() |>
      filter1() |>
      collect_geojson()
    ...
    query <- "SELECT %s FROM (%s)" |>
      sprintf(
        ...
        paste0(
          "SELECT ",
          paste0(c(.include, geom), collapse = ","),
          " FROM ST_Read('",
          dsn,
          "')",
          collapse = " UNION ALL "
...
#' @rdname init
#' @export
init_wetlands <- function(conn = init_conn(),
                          ask = interactive(),
                          dsn = NULL,
  ...
  init_generic(conn,
               ...
               layer,
               geom,
               recordid = "93b413d8-1840-4770-9629-641d74bd1cc6",
               tablename = "WETLANDS")
}

#' @rdname init
#' @export
init_rivers <- function(conn = init_conn(),
                        ask = interactive(),
                        dsn = NULL,
  ...
  init_generic(conn,
               ...
               layer,
               geom,
               recordid = "f7dac054-efbf-402f-ab62-6fc4b32a619e",
               tablename = "RIVERS")
}

#' @rdname init
#' @export
init_lakes <- function(conn = init_conn(),
                       ask = interactive(),
                       dsn = NULL,
  ...
  init_generic(conn,
               ...
               layer,
               geom,
               recordid = "cb1e3aba-d3fe-4de1-a2d4-b8b6650fb1f6",
               tablename = "LAKES")
}
...
# which CEF Human Disturbance 2021 BTM glaciers and snow is based on
init_glaciers <- function(conn = init_conn(),
                          ask = interactive(),
                          dsn = NULL,
                          layer = "BTM_PLU_V1",
                          geom = "geom") {
  filter1 <- \(x) {
    bcdata::filter(x, PRESENT_LAND_USE_LABEL == "Glaciers and Snow")
  }
  init_generic(
    conn,
    ask,
    ...
    layer,
    geom,
    recordid = "134fdc69-7b0c-4c50-b77c-e8f2553a1d40",
    tablename = "GLACIERS",
    filter1 = filter1
  )
}
...
#' @rdname init
#' @export
init_ccb <- function(conn = init_conn(),
                     ask = interactive(),
                     dsn = NULL,
...
#' @rdname init
#' @export
init_burn <- function(conn = init_conn(),
                      ask = interactive(),
                      dsn = NULL,
                      layer = "WHSE_FOREST_VEGETATION_VEG_BURN_SEVERITY_SP",
                      geom = "geom") {
  filter1 <- \(x) {
    bcdata::filter(x, BURN_SEVERITY_RATING %in% c("High", "Low", "Medium"))
  }
  init_generic(
    conn,
    ask,
    ...
    layer,
    geom,
    recordid = "c58a54e5-76b7-4921-94a7-b5998484e697",
    tablename = "BURN",
    filter1 = filter1,
...
#' @rdname init
#' @export
init_fire <- function(conn = init_conn(),
                      ask = interactive(),
                      dsn = NULL,
...
#'   A vector of TSA_NUMBER_DESCRIPTION values to filter.
...
init_tsa <- function(conn = init_conn(),
  ...
  if (length(tsa_name)) {
    filter1 <- \(x) {
      bcdata::filter(x, TSA_NUMBER_DESCRIPTION %in% tsa_name)
    }
  }
  init_generic(
    conn,
  ...
  if (isTRUE(Skeena_boundary)) {
    filter1 <- \(x) {
      bcdata::filter(x, ORG_UNIT_NAME == "Skeena Natural Resource Region")
    }
    init_generic(
      conn,
      ask,
...
resolve_resource <- function(x) {
  ...
  suppressMessages(untrace(curl:::print_progress))
...
meta_proj4 <- function(dsn, layer, geom_f) {
  ...
  if (missing(layer)) {
    ...
    logger::log_info("Using the first layer found for proj4 [%s]." |>
                       sprintf(layer))
  } else {
    l <- which(vapply(res$layers, `[[`, character(1), "name") == layer)
    if (!length(l)) {
      logger::log_warn("Layer not found in dsn for proj4 [%s: %s]." |>
                         sprintf(dsn, layer))
      return("")
    }
  }
  ...
  if (!length(res$layers[[l]]$geometry_fields)) {
    logger::log_warn(sprintf(
      "Geometry field not found for layer in dsn for proj4 [%s: %s].",
      dsn,
      layer
    ))
    return("")
  ...
  if (missing(geom_f)) {
    ...
    logger::log_info(sprintf(
      "Using the first geometry field found in layer for proj4 [%s: %s].",
      layer,
      geom_f
    ))
  } else {
  ...
  res$layers[[l]]$geometry_fields[[g]]$crs$proj4
}
